...
async function test(workerSid, password1, password2, adminTask) {
  var insertResult = await passwordObj.insertPassword(
    workerSid,
    password1,
    adminTask
  );
  var updateResult = await passwordObj.updatePassword(
    workerSid,
    password2,
    adminTask
  );
  var matchResult = await passwordObj.verifyPassword(
    workerSid,
    password2,
    adminTask
  );
  console.log("matchResult: " + matchResult);
  matchResult = await passwordObj.verifyPassword(
    workerSid,
    password1,
    adminTask
  );
