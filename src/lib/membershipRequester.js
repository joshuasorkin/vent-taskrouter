...
class MembershipRequester {
  ...
  sendMessageToContact_uri(contact_uri, messageBody) {
    ...
    this.client.messages
      ...
      .then((message) =>
        console.log(
          "MembershipRequester.sendMessageToWorkerEntity(): sent message to contact_uri " +
            contact_uri +
            ": " +
            message.sid
        )
      ...
      .catch((err) =>
        console.log(
          "MembershipRequester.sendMessageToWorkerEntity(): Error sending message to contact_uri " +
            contact_uri +
            ": " +
            err
        )
  ...
  async requestNewWorker(contact_uri, friendlyName) {
    ...
    if (this.dataValidator.validPhoneNumber(contact_uri)) {
      ...
      try {
        workerEntity = await this.worker.getWorkerEntityFromContact_uri(
          contact_uri
        );
        console.log(
          "requestNewWorker: finished getWorkerEntityFromContact_uri"
        );
      ...
      if (notFound) {
        try {
          ...
          workerEntity =
            this.worker.getWorkerEntityFromFriendlyName(friendlyName);
          //notFound=(workerEntity==null);
          //todo: delete below line, replace with above line when finished testing
          notFound = true;
          if (notFound) {
            var authenticateCode = this.getRndInteger(
              process.env.AUTHENTICATECODE_MIN,
              process.env.AUTHENTICATECODE_MAX
            );
            console.log(
              "requestNewWorker: authenticate code is " + authenticateCode
            );
            console.log(
              "requestNewWorker: running worker.createWorkerApply..."
            );
            ...
            var messageText =
              "A request has been made to add you as a Vent user with the name " +
              friendlyName +
              "." +
              "  To confirm your identity, please respond to this text with the number " +
              authenticateCode +
              ".";
  ...
  async verifyRequest(contact_uri, authenticateCode) {
    var membershipRequest = await this.worker.getMembershipRequest(
      contact_uri,
      authenticateCode
    );
    if (membershipRequest == null) {
      ...
      console.log(
        "verifyRequest: membershipRequest: " + JSON.stringify(membershipRequest)
      );
      console.log(
        "verifyRequest: friendlyName for this request is: " +
          membershipRequest.friendlyname
      );
      ...
      if (workerEntity == null) {
        var updateResult = await this.worker.updateMembershipRequestToComplete(
          fromNumber
        );
        var addResult = await this.sms.addWithoutParameterObj(
          contact_uri,
          friendlyName
        );
        ...
        return (
          "A worker already exists with the name corresponding to this authentication code, " +
